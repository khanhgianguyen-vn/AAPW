...
def fetch_pending_accounts():
    if not NIXUS_API_URL or not AAPW_API_KEY:
        log.error("NIXUS_API_URL or AAPW_API_KEY not set — check your .env")
        return []
    try:
        r = requests.get(f"{NIXUS_API_URL}/api/aapw/accounts", headers=_headers(), timeout=15)
        r.raise_for_status()
        ...
        for acct in accounts:
            ...
            try:
                ...
                if result.get("status") == "success":
                    ...
                    report_result(email_account_id, result.get("app_password"), "success")
                else:
